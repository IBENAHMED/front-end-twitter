...
export const EditProfileModal = () => {
  const editProfileModal: any = useRef(null)
  ...
  const {cookies, sideBarEffect, setSideBarEffect} = useContext(userContextProvider)

  const handlingEditProfile = async (e: any) => {
    e.preventDefault()
    try {
      let res
      try {
  ...
  return (
    <>
      <button className='btn btn-outline rounded-full btn-sm' onClick={() => editProfileModal.current.showModal()}>
        Edit profile
      </button>
      <dialog ref={editProfileModal} id='edit_profile_modal' className='modal'>
        <div className='modal-box border rounded-md border-gray-700 shadow-md'>
          <h3 className='font-bold text-lg my-3'>Update Profile</h3>
          <form className='flex flex-col gap-4' onSubmit={handlingEditProfile}>
            <div className='flex flex-wrap gap-2'>
              <input type='text' placeholder='Full Name' className='flex-1 input border border-gray-700 rounded p-2 input-md' value={formData.fullName} name='fullName' onChange={handleInputChange} />
            </div>
            <div className='flex flex-wrap gap-2'>
              <input type='email' placeholder='Email' className='flex-1 input border border-gray-700 rounded p-2 input-md' value={formData.email} name='email' onChange={handleInputChange} />
            ...
            <div className='flex flex-wrap gap-2'>
              ...
              />
              <input type='password' placeholder='New Password' className='flex-1 input border border-gray-700 rounded p-2 input-md' value={formData.newPassword} name='newPassword' onChange={handleInputChange} />
            </div>
            ...
            <button className='btn btn-primary rounded-full btn-sm text-white'>{isPanding ? "Loading..." : "Update"}</button>
          </form>
        </div>
...
const ProfileItem = ({username}: any) => {
  const {user, cookies, sideBarEffect, setSideBarEffect} = useContext(userContextProvider)
  
  const [feedType, setFeedType] = useState("posts")
  const [coverImg, setCoverImg] = useState(null)
  const [profileImg, setProfileImg] = useState(null)
  const [isLoading, setIsLoading]: any = useState(false)
  const [followUnfollow, setFollowUnfollow]: any = useState()
  const [isLoadingFollwo, setIsLoadingFollwo]: any = useState(false)
  const [userCurrent, setUserCurrent]: any = useState()
  const [feedTypeDetected, setFeedTypeDetected]: any = useState(`/user/${username}`)
          ...
          {!isLoading && userCurrent && (
            <>
              ...
              <div className='relative group/cover'>
                <img src={(coverImg && coverImg) || userCurrent?.coverImg || cover.src} className='h-52 w-full object-cover' alt='cover image' />
                {isMyProfile && (
                  <div
                ...
                <input type='file' hidden ref={coverImgRef} onChange={handleCoverImg} />
                <input type='file' hidden ref={profileImgRef} onChange={handleProfileImg} />
                {/* USER AVATAR */}
              ...
              <div className='flex justify-end px-4 mt-5'>
                ...
                {!isMyProfile && (
                  <button className='btn btn-outline rounded-full btn-sm' onClick={() => handlingFollow(userCurrent._id)}>
                    {followUnfollow && followUnfollow ? "Unfollow" : "Follow"}
                  </button>
                )}
                {(coverImg || profileImg) && (
                  <button className='btn btn-primary rounded-full btn-sm text-white px-4 ml-2' onClick={() => handlingUploadImg()}>
                  ...
                  {userCurrent?.link && (
                    <div className='flex gap-1 items-center '>
                      <>
                        ...
                        <a href={userCurrent?.link} target='_blank' rel='noreferrer' className='text-sm text-blue-500 hover:underline'>
                          youtube.com
                        </a>
                      </>
                    </div>
